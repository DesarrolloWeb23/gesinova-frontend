...
export class TransferTurn {
    constructor(private transferRepository: TransferRepository) {}

    async execute(turnId: number, serviceId: string): Promise<Turn> {
        try {
            const response = await this.transferRepository.transferTurn(turnId, serviceId);
